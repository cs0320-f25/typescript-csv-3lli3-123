...
test ("parseCSV on empty csv throws error", async () => {
  await expect(parseCSV(EMPTY_CSV_PATH)).rejects.toThrow("Empty CSV file, nothing to parse");
});

// test parser on an empty csv file which should throw an error
// to alert the user that the file is empty
test ("parseCSV on headers only csv throws error", async () => {
  await expect(parseCSV(HEADERS_ONLY_CSV_PATH)).rejects.toThrow("No data rows, nothing to parse");
});
...
test ("parseCSV on rows with different columns throws error on inconsistent columns", async () => {
  await expect(parseCSV(INCONSISTENT_COLUMNS_CSV_PATH)).rejects.toThrow("Inconsistent number of columns");
});
...
test ("parseCSV on rows with different datatypes throws error on inconsistent datatypes", async () => {
  await expect(parseCSV(DIFFERENT_DATATYPE_CSV_PATH)).rejects.toThrow("Inconsistent data types in column");
});
...
test ("parseCSV on semicolon delimited csv still parses", async () => {
 const group = await parseCSV(SEMICOLON_CSV_PATH)
...
test ("parseCSV on csv with blank rows doesn't parse", async () => {
   await expect(parseCSV(BLANK_ROW_CSV_PATH)).rejects.toThrow("Gaps between rows are not allowed");
});
...
test ("parseCSV on spaced out csv still parses", async () => {
 const clique = await parseCSV(SPACED_OUT_CSV_PATH)
